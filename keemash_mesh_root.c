				...
				uint8_t status, const char *text)
{
	...
	for (uint8_t i = 0; i < COMMAND_RESULT_CACHE_SIZE; i++) {
		...
		bool identity_matches = operation_id
			? s_command_results[i].has_operation_id &&
			  operation_id_eq(&s_command_results[i].operation_id,
					  operation_id)
			: !s_command_results[i].has_operation_id &&
                           ...
                           uint32_t stream_id)
{
	...
	if (channel == MESH_V2_TUNNEL_CHANNEL_NODEINFO &&
		...
		keemash_mesh_root_on_node_seen_uptime(peer, tag, true, p->uptime_s);
		return;
	}
	if (channel == MESH_V2_TUNNEL_CHANNEL_LOG &&
		...
		keemash_mesh_root_on_node_seen(peer, tag);
		keemash_mesh_root_on_log_line(peer, tag, line);
		return;
	}
	if (channel == MESH_V2_TUNNEL_CHANNEL_TASK &&
	    payload_len >= sizeof(mesh_v2_task_snapshot_payload_t)) {
		keemash_mesh_root_on_task_snapshot(peer, payload);
		return;
	}
	...
	if (channel == MESH_V2_TUNNEL_CHANNEL_TOPOLOGY &&
		...
		keemash_mesh_root_on_topology(peer, payload, payload_len);
		return;
	}
	if (channel == MESH_V2_TUNNEL_CHANNEL_CONTROL &&
		...
		if ((p->kind == MESH_V2_CONTROL_RESULT ||
			...
			if ((p->rsv & MESH_V2_CONTROL_FLAG_OPERATION_ID) &&
			    !has_operation_id) {
				return;
			}
                                    ...
                                    const void *payload, size_t payload_len)
{
	...
	return keemash_mesh_transport_send(mac, buf, sizeof(mesh_v2_hdr_t) + payload_len);
}

static esp_err_t send_control(const uint8_t to[6], const uint8_t mac[6], uint8_t type,
...
static void handle_hello(const uint8_t from[6], const mesh_v2_hdr_t *h, const uint8_t *payload)
{
	...
	keemash_mesh_root_on_node_seen_uptime(h->src_mac, tag, true, uptime_s);
...
static void deliver_legacy_v2_payload(const mesh_v2_hdr_t *h, const uint8_t *payload)
{
	if (h->type == MESH_V2_TYPE_NODEINFO) {
		...
		char tag[MESH_V2_TAG_MAX + 1];
		copy_packet_text(tag, sizeof(tag), p->tag, sizeof(p->tag));
		keemash_mesh_root_on_node_seen_uptime(h->src_mac, tag, true, p->uptime_s);
		return;
	}
	...
	if (h->type == MESH_V2_TYPE_LOG_LINE) {
		...
		copy_packet_text(tag, sizeof(tag), p->tag, sizeof(p->tag));
		copy_packet_text(line, sizeof(line), p->line, sizeof(p->line));
		keemash_mesh_root_on_node_seen(h->src_mac, tag);
		keemash_mesh_root_on_log_line(h->src_mac, tag, line);
	}
}
...
static void deliver_tunnel_payload(const mesh_v2_tunnel_hdr_t *t, const uint8_t *payload)
{
	if (t->channel_id == MESH_V2_TUNNEL_CHANNEL_NODEINFO) {
		...
		char tag[MESH_V2_TAG_MAX + 1];
		copy_packet_text(tag, sizeof(tag), p->tag, sizeof(p->tag));
		keemash_mesh_root_on_node_seen_uptime(t->origin_mac, tag, true, p->uptime_s);
		return;
	}
	...
	if (t->channel_id == MESH_V2_TUNNEL_CHANNEL_LOG) {
		...
		copy_packet_text(tag, sizeof(tag), p->tag, sizeof(p->tag));
		copy_packet_text(line, sizeof(line), p->line, sizeof(p->line));
		keemash_mesh_root_on_node_seen(t->origin_mac, tag);
		keemash_mesh_root_on_log_line(t->origin_mac, tag, line);
		return;
	}
	...
	if (t->channel_id == MESH_V2_TUNNEL_CHANNEL_TASK) {
		...
		keemash_mesh_root_on_task_snapshot(t->origin_mac, p);
		return;
	}
	...
	if (t->channel_id == MESH_V2_TUNNEL_CHANNEL_TOPOLOGY) {
		...
		keemash_mesh_root_on_topology(t->origin_mac, p, t->payload_len);
	}
}

static void handle_lost(const uint8_t from[6], const mesh_v2_hdr_t *h, const uint8_t *payload)
{
...
static void handle_reliable_legacy_v2(const uint8_t from[6], const mesh_v2_hdr_t *h,
                                      const uint8_t *payload)
{
	bool deliver = false;
	bool send_nack = false;
...
esp_err_t mesh_v2_root_send_command(const uint8_t mac[6], uint32_t command_id,
				    const char *command)
{
	return mesh_v2_root_send_command_operation(mac, command_id, command, NULL);
}
	...
	const mesh_v2_operation_id_t *operation_id)
{
	...
	if (operation_id && operation_id->high == 0 && operation_id->low == 0) {
		return ESP_ERR_INVALID_ARG;
	}
	if (operation_id &&
	    !mesh_v2_root_peer_advertises_lossless(mac,
	...
	mesh_v2_control_payload_t *payload = (mesh_v2_control_payload_t *)wire;
	...
	size_t payload_len = keemash_mesh_control_payload_size(
		payload->text_len, operation_id != NULL);
	if (payload_len == 0 || payload_len > sizeof(wire)) {
		xSemaphoreGiveRecursive(s_rel_lock);
		return ESP_ERR_INVALID_SIZE;
	}
	if (operation_id) {
		payload->rsv |= MESH_V2_CONTROL_FLAG_OPERATION_ID;
		memcpy(wire + payload_len - sizeof(*operation_id), operation_id,
		       sizeof(*operation_id));
	}
	esp_err_t err = keemash_rel_send(s_rel, mac,
		MESH_V2_TUNNEL_CHANNEL_CONTROL, wire, payload_len,
		KEEMASH_REL_PRIORITY_CONTROL);
	xSemaphoreGiveRecursive(s_rel_lock);
	return err;
}
				...
				mesh_v2_command_result_t *out)
{
	...
	out->has_operation_id = slot->has_operation_id;
	out->operation_id = slot->operation_id;
}
...
bool mesh_v2_root_command_result_for_operation(
	...
	mesh_v2_command_result_t *out)
{
	if (!mac || !operation_id || !out ||
	    (operation_id->high == 0 && operation_id->low == 0)) return false;
	memset(out, 0, sizeof(*out));
	keemash_rel_stats_t rel = {0};
	bool have_rel = current_rel_stats(mac, &rel);
	portENTER_CRITICAL(&s_lock);
	for (uint8_t i = 0; i < COMMAND_RESULT_CACHE_SIZE; i++) {
		const command_result_slot_t *slot = &s_command_results[i];
		if (!slot->used || !slot->has_operation_id ||
		    !operation_id_eq(&slot->operation_id, operation_id) ||
		    !mac_eq(slot->peer, mac) ||
		    (have_rel &&
		     (slot->root_session_id != rel.root_session_id ||
		      slot->node_session_id != rel.node_session_id))) continue;
